...
<head>
    ...
    <title>Kegiatan | Pesantren Baitul Hikmah</title>
...
<?php  include 'kontrol/database.php' ?>
    ...
    <section id="keg" class="container">
        <ul class="faq unstyled">
        <?php 
                    $baca = mysql_query("select * from tb_kegiatan");
                    $no=1;
                    while ($lihat=mysql_fetch_array($baca)) {
                ?>
            <li>
                <span class="number"><?php echo $no; ?></span>
                <!--<img src="kontrol/admin/gmb/keg/<?php echo $lihat['gbr_kgt']; ?>" class="img-responsive img-thumbnail" height="180" width="150">-->
                <div>
                    <h4><?php echo $lihat['nama_kgt']; ?></h4>
                    <p><?php echo $lihat['isi_kgt']; ?></p>
                </div>
            </li>
            <?php
            $no=$no+1;
